...
from notasDebitoCredito import extDebitoCredito
from sumasEstadoCuenta import sumasEC

def extEstadoCuenta(archivoEscritura, estCuenta):
    #Apertura del archivo para escritura de informacion
    #filesheet = "conciliacion.xlsx"
    wb1 = load_workbook(archivoEscritura)
    hojaEscritura = wb1.worksheets[1]
    #estCuenta = 'estadodeCuenta.xml'
    #Apertura de archivos para extraccion de informacion
    tree = ElementTree.parse(estCuenta)
    root = tree.getroot()
    #suma = 0
    doc = minidom.parse(estCuenta)
    ...
    wb1.save(archivoEscritura)
    
    extDebitoCredito(archivoEscritura, estCuenta)
    sumasEC(archivoEscritura)
